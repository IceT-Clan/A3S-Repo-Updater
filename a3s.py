...
class Updater(object):
    """download and Update a mod from a specified location"""
    def __init__(self, moddir, args):
        super(Updater, self).__init__()
        self.moddir = moddir
        self.args = args
        self.mod = "mod"
        self.displayname = "displayname"
        self.github_loc = "github_loc"
        self.file_format = "file_format"
        self.cur_version = "cur_version"

    def github_release(self, displayname, github_loc, file_format, cur_version):
        ...
        printstatus(0, self.displayname)

        if os.path.isdir(self.displayname):
            modrepo = git.Repo(self.displayname)
        ...
        if new_tag == self.cur_version and not self.args.skip_version:
            # No update needed
            printstatus(1, self.displayname)
            return

        # Remove old Mod
        if os.path.isdir(os.path.join(self.moddir, "@" + self.displayname)):
            shutil.rmtree(os.path.join(self.moddir, "@" + self.displayname))

        # Download newest version
        zipname = self.file_format.replace("$version", new_version)
        savedfile = self.displayname + ".zip"
        ...
        with zipfile.ZipFile(savedfile, "r") as packed:
            for zipinfo in packed.namelist():
                target_dir = packed.extract(zipinfo, self.moddir)
        target_dir = target_dir.replace(self.moddir, '')
        target_dir = target_dir.split('/')[1]
        debug("rename " + self.moddir + "/" + target_dir + " to " +
              self.moddir + "/" + "@" + self.displayname)
        os.rename(self.moddir + "/" + target_dir,
                  self.moddir + "/" + "@" + self.displayname)
        os.remove(savedfile)  # Remove .zip file

        # Write new version to config
        old_line = ",".join([str(x) for x in self.mod])
        new_line = old_line.replace(self.cur_version, new_tag)
        for line in fileinput.input("repo.cfg", inplace=1):
            if old_line in line:
                line = line.replace(old_line, new_line)
            sys.stdout.write(line)

        printstatus(2, self.displayname)

    def github(self, displayname, cur_version):
        """Update github mod"""
        self.displayname = displayname
        self.cur_version = cur_version

    def curl_folder(self, displayname, cur_version):
        """download from loose folder and upate mod"""
        self.displayname = displayname
        self.cur_version = cur_version

    def curl_sort_compressed(self, displayname, file_format, cur_version, sort_by):
        self.displayname = displayname
        self.file_format = file_format
        self.cur_version = cur_version
...
def main():
    ...
    parser.add_argument("-v", "--version",
                        action="version", version='%(prog)s 0.5.0')
    ...
    for mod in modlist:
        ...
        if args.update:
            if mod[0] == "github-release" and github_enabled:
                updater.github_release(mod[1], mod[2], mod[3], mod[4])
            # # Github Release
            # if mod[0] == "github-release" and github_enabled:
            #     displayname = mod[1]
            #     github_loc = mod[2]
